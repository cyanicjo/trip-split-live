...
- 일정 호환 테스트: `test/itinerary.test.js`
...
- 친구 목록의 계좌 정보 클릭 복사
- 지출 입력을 팝업처럼 열기
- 출발일·종료일을 설정해 일차 자동 생성
- PC에서 여러 일차를 가로 열로 비교하고 모바일에서 일차 탭으로 전환
- 일차마다 세로 타임라인과 하루 총지출 표시
- 이동·숙소·식비·그 외 대분류와 기존 카테고리 세부분류 유지
- 같은 일차 안 카드 순서 변경 및 다른 일차 이동
- 숙소를 여러 일차에 이어 표시하되 정산에는 한 번만 포함
- 추가 옵션을 열면 시작·종료 날짜가 같은 날로 자동 설정되고, 범위를 늘리면 총액을 날짜 수만큼 균등 배분해 표시
- 나누어떨어지지 않는 원 단위는 앞 일차부터 1원씩 배분
- 숙소를 포함한 모든 새 지출은 같은 날짜 범위로 시작하며 사용자가 종료 날짜를 늘릴 수 있음
...
- CSV 날짜를 일차로 변환하고 누락 시 미리보기에서 일차 선택
- CSV/PDF/JSON 내보내기에 일차·대분류·식사·숙박 기간 포함
- 해외여행/외화 정산 옵션
- 외화 옵션 접기
- 외화 2개 사용
- 원화 기준 정산
...
- 새 지출은 내용, 금액, 결제자, n빵 참여자만 보이는 간단 모드로 시작합니다.
- 간단 모드도 내부적으로 수량 1인 품목 하나를 저장하므로 기존 정산 및 내보내기 구조와 호환됩니다.
- `여러 품목`을 켜면 품목명, 수량, 단가, 품목별 카테고리와 참여자를 입력할 수 있습니다.
- 품목이 두 개 이상이면 하나만 남길 때까지 간단 모드로 돌아갈 수 없습니다.
- 일차, 시작·종료 날짜, 분류, 외화, 환율, 카드 청구액, 메모, 카테고리 관리는 `추가 옵션`에 있습니다.
